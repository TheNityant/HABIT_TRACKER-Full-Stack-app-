...
def detect_card_genes(sequence, card_fasta_path):
    card_db = load_card_database(card_fasta_path)
    if not card_db:
        return []
    detected_genes = []
...
# --- Print results like before, but advanced ---
if detected_genes:
    ...
    for name, data in list(unique_genes.items())[:10]:
        print(f" 🧬 [{data['aro_id']}] {data['gene_name']}")
